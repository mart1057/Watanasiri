<template>
    <div>
        ...
        <v-row>
            ...
            <v-col md="1">
                <vs-button flat @click="getProducMaintList()" >ค้นหา</vs-button>
            </v-col>
            <v-col md="3">
        ...
        <v-row>
            <h6>จัดการข้อมูล ประเภทสินค้าหลัก</h6>
        </v-row>
        <v-row>
            <vs-table>
                <template #thead>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in items[0]" :data="item">
                        ...
                        <vs-td>{{ item.attributes.product_maincate_code }}</vs-td>
                        <vs-td>{{ item.attributes.product_maincate_name }}</vs-td>
                ...
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="รหัสประเภทสินค้าหลัก *" v-model="formItem.code" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="ชื่อประเภทสินค้าหลัก *" v-model="formItem.name" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <label>สถานะ *</label>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'รหัสประเภทสินค้าหลัก', value: 'code' },
                { text: 'ชื่อประเภทสินค้าหลัก', value: 'name' },
    ...
    methods: {
        ...
        getProducMaintList() {
            this.items=[]
            fetch(process.env.VUE_APP_BACKEND + 'product-main-categories?populate=*&filters[product_maincate_name][$contains]='+this.filter.text+'&pagination[page]='+this.page+'&pagination[pageSize]=10')
        ...
        getProductMainDetail(id) {
            this.is_edit = true
            fetch(process.env.VUE_APP_BACKEND + 'product-main-categories/' + id)
                .then(response => response.json())
                .then((resp) => {
                    this.formItem.name = resp.data.attributes.product_maincate_name
                    this.formItem.status = resp.data.attributes.status
                    this.formItem.code = resp.data.attributes.product_maincate_code
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'product-main-categories/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        saveOrEditProduct() {
            if (this.is_edit == true) {
                ...
                axios.put(process.env.VUE_APP_BACKEND + 'product-main-categories/' + this.formItem.id, {
                    "data": {
                        "product_maincate_code": this.formItem.code,
                        "product_maincate_name": this.formItem.name,
                ...
                axios.post(process.env.VUE_APP_BACKEND + 'product-main-categories', {
                    "data": {
                        "product_maincate_code": this.formItem.code,
                        "product_maincate_name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data){
                axios.put(process.env.VUE_APP_BACKEND + 'product-main-categories/' + data.id, {
                    "data": {
