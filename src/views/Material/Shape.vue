<template>
    <div>
        <v-row>
            <h2>รูปร่างโลหะ/ลักษณะวัตถุดิบ/แผนกวัสดุสิ้นเปลือง</h2>
        </v-row>
        <v-row>
            <v-col md="1">
        ...
        <v-row>
            <h6>จัดการข้อมูลรูปร่างโลหะ/ลักษณะวัตถุดิบ/แผนกวัสดุสิ้นเปลือง</h6>
        </v-row>
        <v-row>
            <vs-table>
                <template #thead>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in items[0]" :data="item">
                        ...
                        <vs-td>{{ item.attributes.metal_shape_code }}</vs-td>
                        <vs-td>{{ item.attributes.metal_shape_name }}</vs-td>
                            ...
                            <vs-tr>
                                <vs-td class="pa-0">
                                    <vs-button transparent
                                        @click.prevent="getMaterialShapeDetail(item.id), title = 'แก้ไข'">
                ...
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="รหัสรูปร่างโลหะ *" v-model="formItem.code" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="ชื่อรูปร่างโลหะ *" v-model="formItem.name" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <label>สถานะ *</label>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'รหัสรูปร่างโลหะ', value: 'code' },
                { text: 'ชื่อรูปร่างโลหะ', value: 'name' },
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'metal-shapes/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        getMaterialShapeDetail(id) {
            this.isEdit = true
            fetch(process.env.VUE_APP_BACKEND + 'metal-shapes/' + id)
                .then(response => response.json())
                .then((resp) => {
                    console.log(resp);
                    this.formItem.name = resp.data.attributes.metal_shape_name
                    this.formItem.code = resp.data.attributes.metal_shape_code
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'metal-shapes/' + this.formItem.id, {
                    "data": {
                        "metal_shape_code": this.formItem.code,
                        "metal_shape_name": this.formItem.name,
                        "status": this.formItem.status
                    }
                })
            }
            else {
                axios.post(process.env.VUE_APP_BACKEND + 'metal-shapes', {
                    "data": {
                        "metal_shape_code": this.formItem.code,
                        "metal_shape_name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data) {
            axios.put(process.env.VUE_APP_BACKEND + 'metal-shapes/' + data.id, {
                "data": {
