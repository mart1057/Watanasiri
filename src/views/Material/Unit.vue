<template>
    <div>
            ...
            <vs-table>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in items[0]" :data="item">
                        ...
                        <vs-td>{{ item.attributes.unit_code}}</vs-td>
                        <vs-td>{{ item.attributes.unit_name }}</vs-td>
        ...
        <vs-dialog v-model="dialog">
            <template #header>
                <h4 class="not-margin">{{ title }} จัดการข้อมูลหน่วย</h4>
            </template>
            <div class="con-form">
                <v-container>
                    <v-row>
...
export default {
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'units/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        getUnitDetail(id) {
            ...
            fetch(process.env.VUE_APP_BACKEND + 'units/' + id)
                ...
                .then((resp) => {
                    console.log(resp);
                    this.formItem.name = resp.data.attributes.unit_name
                    this.formItem.code = resp.data.attributes.unit_code
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'units/' + this.formItem.id, {
                    "data": {
                        "unit_code": this.formItem.code,
                        "unit_name": this.formItem.name,
                        "status": this.formItem.status
                    }
                })
            }
            else {
                axios.post(process.env.VUE_APP_BACKEND + 'units', {
                    "data": {
                        "unit_code": this.formItem.code,
                        "unit_name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data) {
            axios.put(process.env.VUE_APP_BACKEND + 'units/' + data.id, {
                "data": {
