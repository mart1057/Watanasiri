<template>
    <div>
        <v-row>
            <h2>ข้อมูลรหัสลักษณะพิเศษวัสดุสิ้นเปลือง</h2>
        </v-row>
        <v-row>
            <v-col md="1">
        ...
        <v-row>
            <h6>จัดการข้อมูลรหัสลักษณะพิเศษวัสดุสิ้นเปลือง</h6>
        </v-row>
        <v-row>
            <vs-table>
                <template #thead>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in $vs.getPage(items[0], page, max)" :data="item">
                        ...
                        <vs-td>
                            <vs-tr>
                                <vs-td class="pa-0">
                                    <vs-button transparent @click.prevent="getSuppliesDetail(item.id), title = 'แก้ไข'">
        ...
        <vs-dialog v-model="dialog">
            <template #header>
                <h4 class="not-margin">{{ title }} จัดการข้อมูลรหัสลักษณะพิเศษวัสดุสิ้นเปลือง</h4>
            </template>
            <div class="con-form">
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="รหัสลักษณะพิเศษวัสดุสิ้นเปลือง *" v-model="formItem.code" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="ชื่อลักษณะพิเศษวัสดุสิ้นเปลือง *"  v-model="formItem.name" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <label>สถานะ *</label>
...
import CardSpecialConsumables from "@/views/Material/CardSpecialConsumables.vue";

export default {
    name: 'SpecialConsumables',
    components: {
        CardSpecialConsumables,
    },
    data() {
        return {
            page: 1,
            ...
            headers: [
                ...
                { text: 'รหัสลักษณะพิเศษวัสดุสิ้นเปลือง', value: 'code' },
                { text: 'ชื่อลักษณะพิเศษวัสดุสิ้นเปลือง', value: 'name' },
    ...
    mounted() {
      ...
      methods: {
        ...
        getSupplies() {
            fetch(process.env.VUE_APP_BACKEND + 'supplies')
        ...
        getSuppliesDetail(id) {
            this.isEdit = true
            fetch(process.env.VUE_APP_BACKEND + 'supplies/' + id)
                .then(response => response.json())
                .then((resp) => {
                    console.log(resp);
                    this.formItem.name = resp.data.attributes.supplies_name
                    this.formItem.code = resp.data.attributes.supplies_code
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'supplies/' + this.formItem.id, {
                    "data": {
                        "supplies_code": this.formItem.code,
                        "supplies_name": this.formItem.name,
