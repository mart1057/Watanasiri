                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in items[0]" :data="item">
                        ...
                        <vs-td>
                            <vs-tr>
                                <vs-td class="pa-0">
                                    <vs-button transparent @click.prevent="getWideDetail(item.id), title = 'แก้ไข'">
...
export default {
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'material-widths/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        getWideDetail(id) {
            this.isEdit = true
            fetch(process.env.VUE_APP_BACKEND + 'material-widths/' + id)
                .then(response => response.json())
                .then((resp) => {
                    console.log(resp);
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'material-widths/' + this.formItem.id, {
                    "data": {
                ...
                axios.post(process.env.VUE_APP_BACKEND + 'material-widths', {
                    "data": {
                        "code": this.formItem.code,
                        "name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data) {
            axios.put(process.env.VUE_APP_BACKEND + 'material-widths/' + data.id, {
                "data": {
