<template>
    <div>
        ...
        <v-row>
            ...
            <v-col md="1">
                <vs-button flat @click="getLength()">ค้นหา</vs-button>
            </v-col>
            <v-col md="3">
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                // { text: 'แปลงหน่วยเป็น มม.', value: 'convert' },
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'material-lengths/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        getLength() {
            this.items = []
            fetch(process.env.VUE_APP_BACKEND + 'material-lengths?populate=*&filters[name][$contains]='+this.filter.text+'&pagination[page]='+this.page+'&pagination[pageSize]=10')
        ...
        getLengthDetail(id) {
            this.isEdit = true
            fetch(process.env.VUE_APP_BACKEND + 'material-lengths/' + id)
                .then(response => response.json())
                .then((resp) => {
                    console.log(resp);
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'material-lengths/' + this.formItem.id, {
                    "data": {
                ...
                axios.post(process.env.VUE_APP_BACKEND + 'material-lengths', {
                    "data": {
                        "code": this.formItem.code,
                        "name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data) {
            axios.put(process.env.VUE_APP_BACKEND + 'material-lengths/' + data.id, {
                "data": {
