<template>
    <div>
        <v-row>
            <h2>ข้อมูลลักษณะพิเศษของโลหะ/ชื่อเฉพาะวัตถุดิบ/วัสดุสิ้นเปลือง</h2>
        </v-row>
        <v-row>
            <v-col md="1">
        ...
        <v-row>
            <h6>จัดการข้อมูลข้อมูลลักษณะพิเศษของโลหะ/ชื่อเฉพาะวัตถุดิบ/วัสดุสิ้นเปลือง</h6>
        </v-row>
        <v-row>
            <vs-table>
                <template #thead>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in items[0]" :data="item">
                        ...
                        <vs-td>{{ item.attributes.metal_effects_code}}</vs-td>
                        <vs-td>{{ item.attributes.metal_effects_name }}</vs-td>
                        ...
                        <vs-td>
                            <vs-tr>
                                <vs-td class="pa-0">
                                    <vs-button transparent @click.prevent="getMaterialEffectDetail(item.id), title = 'แก้ไข'">
        ...
        <vs-dialog v-model="dialog">
            <template #header>
                <h4 class="not-margin">{{ title }} จัดการข้อมูลลักษณะพิเศษของโลหะ/ชื่อเฉพาะวัตถุดิบ/วัสดุสิ้นเปลือง</h4>
            </template>
            <div class="con-form">
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="รหัสลักษณะพิเศษ *" v-model="formItem.code"/>
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <vs-input type="text" label="ชื่อลักษณะพิเศษ *"  v-model="formItem.name" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <label>สถานะ *</label>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'รหัสลักษณะพิเศษ', value: 'code' },
                { text: 'ชื่อลักษณะพิเศษ', value: 'name' },
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            axios.delete(process.env.VUE_APP_BACKEND + 'metal-effects/' + this.deleteIdItem)
            this.closeDelete()
        },
        close() {
            this.dialog = false
        ...
        getMaterialEffectDetail(id) {
            this.isEdit = true
            fetch(process.env.VUE_APP_BACKEND + 'metal-effects/' + id)
                .then(response => response.json())
                .then((resp) => {
                    console.log(resp);
                    this.formItem.name = resp.data.attributes.metal_effects_name
                    this.formItem.code = resp.data.attributes.metal_effects_code
        ...
        saveOrEdit() {
            if (this.isEdit == true) {
                axios.put(process.env.VUE_APP_BACKEND + 'metal-effects/' + this.formItem.id, {
                    "data": {
                        "metal_effects_code": this.formItem.code,
                        "metal_effects_name": this.formItem.name,
                        "status": this.formItem.status
                    }
                })
            }
            else {
                axios.post(process.env.VUE_APP_BACKEND + 'metal-effects', {
                    "data": {
                        "metal_effects_code": this.formItem.code,
                        "metal_effects_name": this.formItem.name,
                        "status": this.formItem.status
        ...
        changeStatus(data) {
            axios.put(process.env.VUE_APP_BACKEND + 'metal-effects/' + data.id, {
                "data": {
