<template>
    <div>
        ...
        <v-row>
            <h6>จัดการข้อมูล ใบประเมินต้นทุน</h6>
        </v-row>
        <v-row>
            <vs-table>
                <template #thead>
                ...
                <template #tbody>
                    <vs-tr :key="i" v-for="(item, i) in $vs.getPage(items, page, max)" :data="item">
                        <vs-td>{{ i + 1 }}</vs-td>
                        <vs-td>{{ item.estimateCode }}</vs-td>
                        <vs-td>{{ item.amount }}</vs-td>
                        <vs-td>{{ item.totalTax }}</vs-td>
                        <vs-td>{{ item.customer }}</vs-td>
                        <vs-td>{{ item.project }}</vs-td>
                        <vs-td>{{ item.tags }}</vs-td>
                        <vs-td>{{ item.date }}</vs-td>
                        <vs-td>{{ item.expiryDate }}</vs-td>
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'Estimate', value: 'estimateCode' },
